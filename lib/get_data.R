...
# -------------------------------------------------
get_10year <- function( limit ) {


  if ( TRUE ) 
  {
    query  <-  paste0( "
                    select pin, supply_date, quantity, item_code
                    FROM  pbs 
                    " , 
                    ifelse( limit==0, '', paste( 'limit', limit) ))
  } 
  else 
  {
    query  <-  paste0( "
...
# -------------------------------------------------
# -------------------------------------------------
get_disease <- function( ) {

  query  <-  paste0( "
                    SELECT pin, chronic_disease_category, 
                    scripts as n_script, 
                    pills as n_pill
                    FROM people_disease
                    "
...
get_drug <- function( ) {
  ...
  query  <-  paste0( "
    SELECT item_code, type_code, generic_name, type_name, unit_wt, ddd_mg_factor 
    FROM 
    continuing_item i 
    JOIN generictype USING (type_code)"
      )
...
# -------------------------------------------------
my_db_get_query <- function ( query, dbname='mofi' ) {
